// book.cpp — Polyglot opening-book: hashing + .bin reader.
...
// ---------------------------------------------------------------------------
// The published PolyGlot Random64 array (781 constants), copied verbatim from
// Polyglot's random.cpp. Layout:
//   [0   .. 767] piece-square    (64 * 12)
//   [768 .. 771] castling        (WhiteOO, WhiteOOO, BlackOO, BlackOOO)
//   [772 .. 779] en-passant file (a..h)
//   [780]        side to move (white)
// Verified against the official Polyglot test hashes in polyglot_key().
// ---------------------------------------------------------------------------
namespace {
...
uint64_t polyglot_key(const Position& pos) {
    ...
    // Pieces. Polyglot "kind" order:
    //   0 bp, 1 wp, 2 bn, 3 wn, 4 bb, 5 wb, 6 br, 7 wr, 8 bq, 9 wq, 10 bk, 11 wk
    // i.e. kind = 2*(pieceType-PAWN) + (color==WHITE ? 1 : 0).
    // offset = 64*kind + 8*rank + file.
    for (Square s = SQ_A1; s <= SQ_H8; s = Square(s + 1)) {
        Piece pc = pos.piece_on(s);
        if (pc == NO_PIECE)
            continue;
        int kind = 2 * (int(type_of(pc)) - PAWN) + (color_of(pc) == WHITE ? 1 : 0);
        int offset = 64 * kind + 8 * int(rank_of(s)) + int(file_of(s));
        key ^= Random64[offset];
    }

    // Castling rights.
    if (pos.can_castle(WHITE_OO))  key ^= Random64[768 + 0];
    if (pos.can_castle(WHITE_OOO)) key ^= Random64[768 + 1];
    if (pos.can_castle(BLACK_OO))  key ^= Random64[768 + 2];
    if (pos.can_castle(BLACK_OOO)) key ^= Random64[768 + 3];

    // En passant: only when a pawn of the side to move can actually capture onto
    // the ep square. The engine's Position already enforces this rule when it
    // sets epSquare (both in set() and do_move()), so a non-NONE ep square here
    // is always "capturable".
    if (pos.ep_square() != SQ_NONE)
        key ^= Random64[772 + int(file_of(pos.ep_square()))];
...
// Translate a Polyglot 16-bit move + the current position into an engine Move,
// validated against the legal move list. Returns Move::none() if not legal.
Move polyglot_to_engine(uint16_t pgMove, const Position& pos) {
    int toFile   = (pgMove >> 0) & 0x7;
    int toRow    = (pgMove >> 3) & 0x7;
    int fromFile = (pgMove >> 6) & 0x7;
    int fromRow  = (pgMove >> 9) & 0x7;
    int promo    = (pgMove >> 12) & 0x7;  // 0 none, 1 N, 2 B, 3 R, 4 Q

    Square from = make_square(File(fromFile), Rank(fromRow));
    Square to   = make_square(File(toFile), Rank(toRow));

    // Promotion piece type (engine PieceType), if any.
    PieceType promoType = NO_PIECE_TYPE;
    switch (promo) {
        case 1: promoType = KNIGHT; break;
        case 2: promoType = BISHOP; break;
        case 3: promoType = ROOK;   break;
        case 4: promoType = QUEEN;  break;
        default: promoType = NO_PIECE_TYPE; break;
    }

    // Polyglot encodes castling as "king captures own rook" (e.g. e1h1 for
    // white O-O). The engine represents castling as king-to/from with the
    // CASTLING flag (e1g1). We detect a king move that lands on a friendly rook
    // and let the legal-move match below resolve it: rather than hand-building
    // the flagged move, we search the legal list for the move whose semantics
    // match this from/to (with castling remapping).
    Color us = pos.side_to_move();
    bool isKingFromRook =
        (type_of(pos.piece_on(from)) == KING) &&
        (pos.piece_on(to) != NO_PIECE) &&
        (color_of(pos.piece_on(to)) == us) &&
        (type_of(pos.piece_on(to)) == ROOK);

    for (const ExtMove& em : MoveList<LEGAL>(pos)) {
        Move m = em.move;

        if (m.type_of() == CASTLING) {
            // Engine castling move: from = king square, to = king destination
            // (g1/c1 etc.). Match if this Polyglot move is the same castling.
            if (isKingFromRook && m.from_sq() == from) {
                // Confirm side: Polyglot 'to' is the rook square; the engine
                // castling 'to' is the king's two-square destination. Match by
                // whether both are king-side or queen-side (rook file vs king
                // destination file).
                bool pgKingSide     = int(file_of(to)) > int(file_of(from));
                bool engineKingSide = int(file_of(m.to_sq())) > int(file_of(m.from_sq()));
                if (pgKingSide == engineKingSide)
                    return m;
            }
            continue;
        }
        ...
        if (m.type_of() == PROMOTION) {
            if (promoType != NO_PIECE_TYPE && m.promotion_type() == promoType)
                return m;
            continue;
        }

        // Normal or en-passant move (Polyglot has no distinct ep flag; from/to
        // suffice, and the engine's own ep move has the matching from/to).
        if (promoType == NO_PIECE_TYPE)
            return m;
    }
...
bool Book::load(const std::string& path) {
    ...
    size_t n = size_t(size) / 16;

    // Read the whole file, then decode. Reserve exactly n so the entry vector
    // is a single, right-sized allocation (no growth reallocations).
    ...
    for (size_t i = 0; i < n; ++i) {
        const unsigned char* p = buf.data() + i * 16;
        entries_[i].key    = read_be64(p);
        entries_[i].move   = read_be16(p + 8);
        entries_[i].weight = read_be16(p + 10);
        entries_[i].learn  = read_be32(p + 12);
    }
...
Move Book::probe(const Position& pos, bool pickBest) const {
    ...
    // Gather all matching entries whose polyglot move is legal in this position.
    struct Cand { Move move; uint16_t weight; };
    std::vector<Cand> cands;
    uint32_t totalWeight = 0;
    for (size_t i = lo; i < entries_.size() && entries_[i].key == key; ++i) {
        Move m = polyglot_to_engine(entries_[i].move, pos);
        if (m == Move::none())
            continue;
        uint16_t w = entries_[i].weight ? entries_[i].weight : 1;
        cands.push_back({m, w});
        totalWeight += w;
    }
    ...
    if (pickBest) {
        Move best = cands[0].move;
        uint16_t bestW = cands[0].weight;
        for (const Cand& c : cands)
            if (c.weight > bestW) { bestW = c.weight; best = c.move; }
        return best;
    }

    // Weighted-random selection, deterministic per position (PRNG seeded from
    // the polyglot key so a given position always yields the same choice).
    uint64_t rng = key ? key : 0x9E3779B97F4A7C15ULL;
    rng ^= rng >> 12; rng ^= rng << 25; rng ^= rng >> 27;
    rng *= 0x2545F4914F6CDD1DULL;
    uint32_t pick = uint32_t(rng % (totalWeight ? totalWeight : 1));
