/*
  Wizard — Polyglot opening-book: hashing + .bin reader.
...
PolyglotBook Book;  // global instance
...
// Translate a Polyglot 16-bit move + the current position into a Stockfish Move,
// validated against the legal move list. Returns MOVE_NONE if not legal.
//
// Both Polyglot and Stockfish encode castling as "king moves onto its own rook"
// (e.g. white O-O is e1->h1), so the from/to squares match directly and no
// special castling remapping is needed.
Move polyglot_to_move(uint16_t pgMove, const Position& pos) {
    const int toFile   = (pgMove >> 0) & 0x7;
    const int toRow    = (pgMove >> 3) & 0x7;
    const int fromFile = (pgMove >> 6) & 0x7;
    const int fromRow  = (pgMove >> 9) & 0x7;
    const int promo    = (pgMove >> 12) & 0x7;  // 0 none, 1 N, 2 B, 3 R, 4 Q

    const Square from = make_square(File(fromFile), Rank(fromRow));
    const Square to   = make_square(File(toFile),   Rank(toRow));
    ...
    for (const ExtMove& em : MoveList<LEGAL>(pos)) {
        ...
        if (type_of(m) == PROMOTION) {
            if (promoType != NO_PIECE_TYPE && promotion_type(m) == promoType)
                return m;
            continue;
        }

        // NORMAL, ENPASSANT, or CASTLING (from/to already matched above).
...
uint64_t polyglot_key(const Position& pos) {
    ...
    // En passant: Polyglot includes the ep file only when a pawn can actually
    // capture onto the ep square. Stockfish only sets ep_square() under exactly
    // that condition (see Position::do_move), so a non-NONE ep square here is
    // always "capturable" and maps directly to the Polyglot rule.
...
bool PolyglotBook::load(const std::string& path) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        entries[i].key    = read_be64(p);
        entries[i].move   = read_be16(p + 8);
        entries[i].weight = read_be16(p + 10);
        entries[i].learn  = read_be32(p + 12);
    }

    return !entries.empty();
}

Move PolyglotBook::probe(const Position& pos, bool pickBest) const {
    if (entries.empty())
        return MOVE_NONE;
    ...
    for (size_t i = lo; i < entries.size() && entries[i].key == key; ++i) {
        Move m = polyglot_to_move(entries[i].move, pos);
        if (m == MOVE_NONE)
            continue;
        uint16_t w = entries[i].weight ? entries[i].weight : 1;
        cands.push_back({m, w});
        totalWeight += w;
    }
